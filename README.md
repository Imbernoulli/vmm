...
> **当前以本段为准（2026-06-20）：** 我把 Qwen3 MoE unified average 从原来的 route/geometry 风险规则，改成了同时使用 `route mass + router fragility + load + expert geometry + expert subspace conflict` 的同构约束优化。新的 [Qwen3 MoE Unified Mechanism Candidate](results/qwen3_moe_unified_mechanism_candidate/report.md) 在 `28` 个候选 cap/prior 中选中 `subspace_cap_s1.00`，不是旧的 `router_evidence_risk_s0.75`：route-mass nonbase retention `0.9763`，max predicted routed relative delta `0.6438`，hard-cap violations `0`，risk/geometry/subspace weighted predicted delta 分别是 `0.2247/0.2184/0.2148`，高 subspace-conflict expert 的 mean scale 被压到 `0.9614`。这个选择来自真实内部参数：`5,243` 个 routed expert groups、`1,139` 个 high subspace-conflict groups、其中 `242` 个 route-important，且原 unified 仍有 `17` 个 uncovered subspace-conflict groups。现在这个候选已经重新物化到 `results/checkpoints/qwen3_moe_unified_mechanism_candidate`，manifest 和当前 tensor rules 完全一致（rule status `fresh`，max weight diff `0`）；新的 [unified mechanism delta audit](results/qwen3_moe_unified_mechanism_delta_audit/report.md) 已通过：total relative delta norm `0.240378`，routed expert FFN relative norm `0.248668`，router `0/48` changed，attention `0/288` changed，routed `>0.65` 为 `0`。新的 canonical task manifest 也已写到 `results/qwen3_moe_mechanism_eval_gate/task_manifest.json`，preflight 状态是 `task_manifest_ready`，GSM8K/HumanEval/MMLU/safety 样本数为 `384/164/384/384`，13 个 source/candidate eval row 全部对齐同一个 manifest（sha256 `84945db35ae5a366be5df909a53820bbfbfdeaaa5850ca5d7e03e29ebe8e5082`）。新的 [Unified Average Optimizer](results/unified_average_optimizer/report.md) 状态仍是 `built_waiting_for_qwen3_vllm_eval`。下一步已经不再是物化或 manifest，而是用 vLLM 同场评测 source endpoints 和 same-shape candidates。

> **最新（机制驱动 unified average，2026-06）：** 见 **[Unified Average Optimizer](results/unified_average_optimizer/report.md)**、**[Qwen3 MoE vLLM Eval Budget Plan](results/qwen3_moe_eval_budget_plan/report.md)**、**[Qwen3 MoE Expert Geometry Probe](results/qwen3_moe_expert_geometry_probe/report.md)**、**[Qwen3 MoE Router Calibration NLL Probe](results/qwen3_moe_router_calibration_nll_probe/report.md)**、**[Qwen3 MoE Downstream Confidence Audit](results/fp_downstream_confidence_audit/report.md)** 和 **[Qwen3 MoE Unified Mechanism Candidate](results/qwen3_moe_unified_mechanism_candidate/report.md)**。当前 unified 规则已经从 route/geometry-only 更新为 subspace-aware：Dense 仍拒绝默认 midpoint；MoE 先固定 expert identity、冻结 direct router movement，再用 route/evidence/geometry/subspace cap 生成 same-shape expert rules。Qwen3 Instruct/Coder 直线 connectivity 仍不支配 endpoint，router-only NLL probe 说明 dispatch calibration 是真实杠杆但还不是接受条件；下游生成置信审计也显示 router-cal 的 confident source-frontier wins 是 `0/3`。因此当前可交付结果是一个已物化、已 delta-audit 的 `subspace_cap_s1.00` 同构 checkpoint，最终是否比 source endpoints 好，仍必须由同一 task manifest 下的 vLLM eval 决定。
...
最新 [Qwen3 MoE Delta Frontier Probe](results/qwen3_moe_delta_frontier/report.md) 把九个已物化 same-shape candidates 的真实 safetensors delta 放到同一张表里。结论很明确：route-guarded -> audit-gated 主要消掉 routed expert 的危险大步长（`>1.0` 从 `182` 到 `0`，`>0.75` 从 `839` 到 `164`）；audit-gated -> trust-region 继续把 `>0.75` 从 `164` 压到 `14`；expert-only 清零 attention delta；tail-trimmed/searched/layer-chunk 逐步压 routed tail；subspace-aware unified mechanism candidate 把 strict routed `>0.65` 从 `89` 降到 `0`，total relative norm 是 `0.240378`；subspace-scaled ablation 进一步到 `0.239529`，extra norm reduction 是 `0.000849`，routed `>0.65` 仍是 `0`。这些都是结构安全证据，不是下游胜出证明；attention、risk penalty、layer sensitivity、geometry-aware shrink 和 subspace-conflict shrink 是否保留，必须交给同任务 vLLM eval 决定。
