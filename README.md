...
27. **真实 Qwen MoE 已从 gate 推进到本地 checkpoint，并完成文件级 delta audit。** 旧的 [MoE Materialization Pipeline Plan](results/moe_materialization_pipeline_plan/report.md) 负责说明 gate 结构；当前真正进展见 [Checkpoint Materialization Readiness](results/checkpoint_materialization_readiness/report.md) 和 [Materialized Checkpoint Delta Audit](results/qwen3_moe_materialized_delta_audit/report.md)：`qwen3_moe_unified_route_guarded_candidate` 已经从 writer dry-run 推进到 `materialized_checkpoint_exists`，输出目录有 `16` 个 shard、`18,867` 个同构 tensor；delta audit 证明 router `0/48` 改动、changed tensor `10,641`，下一步是 vLLM 下游评测。
...
32. **Checkpoint materialization readiness 已推进到 Qwen3 candidate ready for eval。** [Checkpoint Materialization Readiness](results/checkpoint_materialization_readiness/report.md) 现在显示 `8` 个候选里 `2` 个已 materialize，其中 Qwen3 route-guarded candidate 是 `ready_for_vllm_eval`，0.5B uniform baseline 已完成 vLLM eval；[vLLM Checkpoint Eval Plan](results/vllm_checkpoint_eval_plan/report.md) 显示 `7` 个 eval rows，其中 Qwen3 Instruct source、Qwen3 Coder source、Qwen3 route-guarded candidate 和 0.5B baseline 都是 `ready_to_host`，`2` 个缺 checkpoint，`1` 个 toy 不可加载。
