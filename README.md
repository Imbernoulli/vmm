...
当前最实在的 Qwen3 MoE 结果是：我已经把 route/load probe、category specialization、router fragility 和真实 delta audit 合成一个同构 trust-region checkpoint，并新增一个 [expert-only attention ablation](results/qwen3_moe_expert_only_trust_region_candidate/report.md) 去隔离 shared attention 的作用。这个 ablation 已物化成本地同构 checkpoint，实际 [delta audit](results/qwen3_moe_expert_only_delta_audit/report.md) 通过：attention `0/288` changed、router `0/48` changed、total relative delta norm `0.246`。现阶段不能提前说 attention 冻结一定更好；真正的算法优劣还要看 Qwen3 Instruct/Coder source、route-guarded、audit-gated、trust-region 和 expert-only ablation 在同一套下游任务上的表现。

最新 [Qwen3 MoE Delta Frontier Probe](results/qwen3_moe_delta_frontier/report.md) 把四个已物化 candidate 的真实 safetensors delta 放到同一张表里。结论很明确：route-guarded -> audit-gated 主要消掉 routed expert 的危险大步长（`>1.0` 从 `182` 到 `0`，`>0.75` 从 `839` 到 `164`）；audit-gated -> trust-region 继续把 `>0.75` 从 `164` 压到 `14`；trust-region -> expert-only 不再改善 routed expert tail，只是把 attention delta 从 `0.189` 清零。因此下一版 unified MoE 规则里，trust-region 是安全机制，attention 是否保留是效用问题，必须交给同任务 vLLM eval 决定。
...
27. **真实 Qwen MoE 已从 gate 推进到四个本地 checkpoint、四份文件级 delta audit、逐 tensor trust-region validation 和一个 attention ablation。** 当前真正进展见 [Checkpoint Materialization Readiness](results/checkpoint_materialization_readiness/report.md)、[Materialized Checkpoint Delta Audit](results/qwen3_moe_materialized_delta_audit/report.md)、[Qwen3 MoE Audit-Gated Candidate](results/qwen3_moe_audit_gated_candidate/report.md)、[audit-gated delta audit](results/qwen3_moe_audit_gated_delta_audit/report.md)、[Qwen3 MoE Trust-Region Candidate](results/qwen3_moe_trust_region_candidate/report.md)、[trust-region delta audit](results/qwen3_moe_trust_region_delta_audit/report.md)、[trust-region delta validation](results/qwen3_moe_trust_region_delta_validation/report.md)、[expert-only attention ablation](results/qwen3_moe_expert_only_trust_region_candidate/report.md) 和 [expert-only delta audit](results/qwen3_moe_expert_only_delta_audit/report.md)：`qwen3_moe_unified_route_guarded_candidate`、`qwen3_moe_audit_gated_candidate`、`qwen3_moe_trust_region_candidate` 和 `qwen3_moe_expert_only_trust_region_candidate` 都已物化成本地 `16` shard / `18,867` tensor 同构 checkpoint。trust-region candidate 把 routing/load/category/delta probe 联合成 expert-level cap，缩小 `405/5,243` 个 expert rules；真实 delta audit 显示 routed expert FFN relative delta norm 从 route-guarded 的 `0.293`、audit-gated 的 `0.270` 进一步降到 `0.255`，`>0.75` 的 routed tensors 从 `839`、`164` 降到 `14`，router 仍是 `0/48` 改动；逐 tensor validation 证明预测和真实物化 delta 的最大相对误差只有 `9.3e-05`。expert-only ablation 冻结 `288` 个 shared attention tensor，实际 total relative delta norm 是 `0.246`，attention 和 router 都是 `0` changed，所以它是为下游 eval 准备的机制对照，而不是提前判定 attention 一定有害。下一步是 vLLM 下游评测。
...
32. **Checkpoint materialization readiness 已推进到四个 Qwen3 candidates ready for eval。** [Checkpoint Materialization Readiness](results/checkpoint_materialization_readiness/report.md) 现在显示 `11` 个候选里 `5` 个已 materialize，其中 Qwen3 route-guarded、audit-gated、trust-region 和 expert-only candidates 都是 `ready_for_vllm_eval`，0.5B uniform baseline 已完成 vLLM eval；[vLLM Checkpoint Eval Plan](results/vllm_checkpoint_eval_plan/report.md) 显示 `10` 个 eval rows，其中 Qwen3 Instruct source、Qwen3 Coder source、四个已物化 Qwen3 MoE candidates 和 0.5B baseline 都是 `ready_to_host`，另外两个 MoE candidates 还缺 checkpoint，`1` 个 toy 不可加载。
