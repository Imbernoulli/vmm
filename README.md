...
> **当前以本段为准（2026-06-20）：** 我把 Qwen3 MoE unified average 从原来的 route/geometry 风险规则，改成了同时使用 `route mass + router fragility + load + expert geometry + expert subspace conflict` 的同构约束优化。新的 [Qwen3 MoE Unified Mechanism Candidate](results/qwen3_moe_unified_mechanism_candidate/report.md) 在 `28` 个候选 cap/prior 中选中 `subspace_cap_s1.00`，不是旧的 `router_evidence_risk_s0.75`：route-mass nonbase retention `0.9763`，max predicted routed relative delta `0.6438`，hard-cap violations `0`，risk/geometry/subspace weighted predicted delta 分别是 `0.2247/0.2184/0.2148`，高 subspace-conflict expert 的 mean scale 被压到 `0.9614`。这个选择来自真实内部参数：`5,243` 个 routed expert groups、`1,139` 个 high subspace-conflict groups、其中 `242` 个 route-important，且原 unified 仍有 `17` 个 uncovered subspace-conflict groups。新的 [Unified Average Optimizer](results/unified_average_optimizer/report.md) 状态是 `built_waiting_for_qwen3_materialization_and_vllm_eval`：现有 `results/checkpoints/qwen3_moe_unified_mechanism_candidate` 是上一版 checkpoint，manifest 和当前 tensor rules 不一致（max weight diff `0.051584`），所以不能把旧 delta audit 当成新算法已经物化的证明。下一步顺序必须是：重新运行 `results/qwen3_moe_unified_mechanism_candidate/writer_command.txt` 物化新 `subspace_cap_s1.00`，再跑 `qwen3_moe_unified_mechanism_delta_audit`，最后进入同一 task manifest 的 vLLM source/candidate eval。

> **最新（机制驱动 unified average，2026-06）：** 见 **[Unified Average Optimizer](results/unified_average_optimizer/report.md)**、**[Qwen3 MoE vLLM Eval Budget Plan](results/qwen3_moe_eval_budget_plan/report.md)**、**[Qwen3 MoE Expert Geometry Probe](results/qwen3_moe_expert_geometry_probe/report.md)**、**[Qwen3 MoE Router Calibration NLL Probe](results/qwen3_moe_router_calibration_nll_probe/report.md)**、**[Qwen3 MoE Downstream Confidence Audit](results/fp_downstream_confidence_audit/report.md)**、**[Qwen3 MoE Unified Mechanism Candidate](results/qwen3_moe_unified_mechanism_candidate/report.md)**、**[Qwen3 MoE Mechanism Leverage Map](results/qwen3_moe_mechanism_levers/report.md)** 和 **[Qwen3 MoE Layer/Chunk Coefficient Candidate](results/qwen3_moe_layer_chunk_candidate/report.md)**。这个脚本把 Dense barrier、MoE expert gauge、Qwen3 expert identity、router movement、router margin fragility、router-only NLL calibration、真实专家内部几何、生成评测置信区间和最终 vLLM 选择 gate 合成一个同构操作策略，而不是静态判断哪个算法名最好。当前证据是：(1) Dense midpoint 的二阶 Fisher 预测严重失效，general/code actual-predicted ratio 为 `42.86/26.66`，所以拒绝默认 `0.5/0.5`；(2) 真实 OLMoE gauge 反事实中，同名 average 让 NLL 从 `4.1678` 升到 `9.6588`，恢复 expert 对齐后回到 `4.1678`；(3) Qwen3-30B-A3B Instruct vs Coder 的 expert identity 通过，`48/48` 层 identity-optimal，但 router direct move gate 为 `0/48` 层通过；新的 router margin gate 又显示 `24/48` 层 high-fragility、L17 score `0.752`、min safe-lambda proxy `0.0197`，所以 direct router average 继续 freeze，只允许单独 router-KD/calibration ablation；(4) 真实 expert geometry probe 读取 `18,432` 个 routed expert projection tensor，mean/p05 cosine 为 `0.386/0.118`、mean/p95 relative delta 为 `1.062/1.275`，发现 `931` 个 high internal-geometry-risk experts 和 `204` 个 high route+geometry-risk experts；(5) Qwen3 MoE Instruct/Coder 直线 connectivity 里 best interior worst-NLL 是 `2.5947`，仍差于 best endpoint `2.4757`，Thinking/Coder complementary path 也没有超过 best source，所以不能因为同族/同构就接受 source-to-source midpoint；(6) 真实 router-only NLL probe 只训练 `mlp.gate.weight`，把 linear MoE merge 的 worst-NLL 从 `2.6355` 降到 `2.4140`，code NLL 比两个 source 最好值还低 `0.0139`，说明剩余误差确实有 router dispatch/co-adaptation 成分，但 worst/avg 仍未支配 best source；(7) 下游生成矩阵里 router calibration 对 naive Instruct+Coder average 的点估计在 GSM8K/HumanEval 上为正，但 Wilson 置信审计显示 confident positive tasks 是 `0/3`、confident source-frontier wins 也是 `0/3`，avg gain 的保守区间是 `[-0.170, 0.231]`，所以它只能作为机制修复信号，不能作为接受 average 的充分条件；(8) 新的 unified mechanism optimizer 选中 `router_evidence_risk_s0.75`，已物化成独立同构 checkpoint，audit 显示 total relative norm `0.240`、router `0/48` changed、attention `0/288` changed、routed max relative delta `0.623`，`>0.65` 为 `0`；(9) 九个 Qwen3 same-shape candidates 仍需同场 vLLM eval，现在九个都已物化并可 host；新增 subspace-scaled ablation 已写出本地 `57G`/`16` shard checkpoint，delta audit `passed`，total relative norm `0.239529`、router `0/48` changed、routed `>0.65` 为 `0`，当前 final selector 是 `awaiting_source_eval`、`0/9` eligible；(10) Qwen3 MoE 远端 eval 不再按 `64` 条 smoke 样本硬选，新的预算脚本按 Wilson 95% half-width `0.05` 和 paired sign-test alpha `0.05` 推荐 `384` max examples，当前是 `13` 个 planned source/candidate rows：`11` 个 ready-to-host（2 个 source + 9 个 Qwen3 candidates）、`2` 个 router-calibration active caps 等待物化，总 planned budget `19,968` prompts，当前可直接跑的 ready budget `16,896` prompts；`cap0025/cap005` 已由 router margin gate 从默认预算中 plan-prune，新增 `margin_profile` 候选按每层 safe-lambda proxy 给 router delta 设上限；同时 `13/13` 个 planned rows 已统一到同一个 canonical task manifest `results/qwen3_moe_mechanism_eval_gate/task_manifest.json`，防止最后 paired prediction gate 因 example key 不一致而失效。

新增 [Qwen3 MoE Expert Subspace Conflict Probe](results/qwen3_moe_expert_subspace_conflict_probe/report.md) 把 expert identity 之后的内部子空间风险也落成了 gate：它从真实 Qwen3 `18,432` 个 routed expert projection 的 channel/chunk geometry 计算 subspace conflict，发现 `1,323/6,144` 个 expert 是 high subspace-conflict，其中 `242` 个同时 route-important；当前 unified 规则已经覆盖绝大多数高风险点，只剩 `17` 个 expert 需要额外 subspace scale，物化成 ablation 后总共只减少 Coder weight `0.253078`。这个 ablation 已写成真实 same-shape checkpoint：`5,243` 条 expert tensor rules 命中 `15,729` 个 projection tensors，`48` 个 router tensors 继续 freeze，本地输出 `57G`、`16` 个 safetensors shard；[subspace-scaled delta audit](results/qwen3_moe_subspace_scaled_delta_audit/report.md) 已通过，total relative norm `0.239529`，相对 unified 只额外减少 `0.000100`，routed `>0.65` 仍为 `0`。这说明下一步不是盲目整体降权，而是用 vLLM 下游评测验证这 `17` 个“同名 expert 但内部局部子空间冲突仍没被 cap 覆盖”的 expert 是否真的需要更细 shrink。
...
当前最实在的 Qwen3 MoE 结果是：我已经把 route/load probe、category specialization、router fragility、真实 expert geometry 和真实 delta audit 合成一个同构候选链。现在除了 [searched cap-law candidate](results/qwen3_moe_searched_no_gt065_delta_audit/report.md) 和 geometry-informed [layer/chunk coefficient candidate](results/qwen3_moe_layer_chunk_delta_audit/report.md)，还物化了独立的 [unified mechanism candidate](results/qwen3_moe_unified_mechanism_delta_audit/report.md) 和 [subspace-scaled ablation](results/qwen3_moe_subspace_scaled_delta_audit/report.md)。unified 在 `19` 个候选 cap/prior 里选中 `router_evidence_risk_s0.75`，不是 searched no-gt-0.65 的 alias；真实 safetensors audit 通过：attention `0/288` changed、router `0/48` changed、total relative delta norm `0.240`、routed max relative delta `0.623`、routed `>0.75`/`>0.65`/`>0.6505` 都为 `0`。subspace-scaled 只在剩余 `17` 个局部子空间冲突 expert 上额外 shrink，audit total relative norm `0.239529`、router `0/48` changed、routed `>0.65` 为 `0`。现阶段不能提前说 unified 或 subspace 一定更好；真正的算法优劣还要看 Qwen3 Instruct/Coder source 和 9 个 same-shape candidates 在同一套下游任务上的表现。

最新 [Qwen3 MoE Delta Frontier Probe](results/qwen3_moe_delta_frontier/report.md) 把九个已物化 same-shape candidates 的真实 safetensors delta 放到同一张表里。结论很明确：route-guarded -> audit-gated 主要消掉 routed expert 的危险大步长（`>1.0` 从 `182` 到 `0`，`>0.75` 从 `839` 到 `164`）；audit-gated -> trust-region 继续把 `>0.75` 从 `164` 压到 `14`；trust-region -> expert-only 不再改善 routed expert tail，只是把 attention delta 从 `0.189` 清零；expert-only -> tail-trimmed 才继续压 routed tail，把 `>0.65` 从 `366` 降到 `80`（剩余都在 `0.6505` rounding slop 内）；searched cap-law 把复杂 penalty 换成统一 `0.65` cap，total relative norm `0.248`；geometry-informed layer/chunk 继续把 total relative norm 降到 `0.243`，strict `>0.65` 降到 `89`；unified mechanism candidate 再把 total relative norm 降到 `0.239629`，并把 routed `>0.65` 从 `89` 降到 `0`；subspace-scaled ablation 进一步到 `0.239529`，但 extra norm reduction 只有 `0.000100`，routed `>0.65` 仍是 `0`。下一版 unified MoE 规则里，trust-region/tail-trim 是安全机制，attention、risk penalty、layer sensitivity、geometry-aware shrink 和 subspace-conflict shrink 是否保留都必须交给同任务 vLLM eval 决定。

最新 [Qwen3 MoE Mechanism-Gated vLLM Eval Gate](results/qwen3_moe_mechanism_eval_gate/report.md) 已把这个问题转成可执行评测：两个 source endpoint 加九个已物化 same-shape Qwen3 MoE candidates 都是 `ready_to_host`，包含 `qwen3_moe_subspace_scaled_candidate`。这个 gate 不再问“哪个算法名最好”，而是逐项检验 tail delta cap、route/load trust-region、shared attention ablation、0.65 tail trim、risk penalty simplification、layer/chunk sensitivity、unified mechanism optimizer、subspace conflict ablation 和 endpoint fallback；如果所有 average 候选被 source 支配，selection rule 会返回同构 endpoint/no-average。当前本机 `nvidia-smi` 不可用，所以 selection 状态是 `awaiting_remote_vllm_eval`。

最新 [Qwen3 MoE vLLM Eval Budget Plan](results/qwen3_moe_eval_budget_plan/report.md) 把“正当评测要跑多少”也落成脚本：原 gate 的 `64` examples 只保留为 audit floor，预算版 `run_eval_budget.sh` 会把 ready-to-host 的十一个 source/candidate 的 `--max-examples` 提到 `384`；预算表现在把两个 source endpoint、九个已物化 Qwen3 same-shape candidates 和 router calibration 的 active `cap001`/`margin_profile` 纳入 planned queue，只有两个 router-calibration active caps 还在等待物化。这个数来自两个约束：binary task score 的 95% Wilson half-width 要压到 `0.05` 以内需要 `381` 条，四舍五入到 `384`；paired prediction gate 若要在 alpha `0.05` 下看出约 `5pp` 的净 source advantage，按 `25%` discordance 估计需要约 `248` 条 shared examples。新的 planned prompt budget 是 `19,968`，其中当前可直接 host 的 ready budget 是 `16,896`；active/ready/pending/plan-pruned router caps 是 `2/0/2/2`。因此下一轮远端 vLLM 不是静态 smoke，也不会默认浪费在 margin gate 已拒绝的 `cap0025/cap005` 上，而是能支撑 source dominance、task regression、score-confidence 和 paired-prediction regression 的机制评测；HumanEval 受 `164` 条数据上限限制，selector 会按实际样本数计算区间。

新增 [Qwen3 MoE Eval Task Manifest Preflight](results/qwen3_moe_eval_manifest_preflight/report.md) 把 paired prediction 的输入条件也提前检查：预算表中 `13/13` 个 planned rows 已对齐到 `results/qwen3_moe_mechanism_eval_gate/task_manifest.json`，但这个 canonical manifest 当前还没有实际写出，所以状态是 `task_manifest_missing`、任务充分性 `0/4`。远端 host vLLM 前必须先运行 `prepare_manifest_command.txt` 里的命令生成同一批 GSM8K/MMLU/safety/HumanEval examples；否则后面的 eval bundle audit 会因为缺少 `task_manifest_sha256` 或 paired keys 直接拒绝选择。
...
最新 [Qwen3 MoE Final Candidate Selector](results/qwen3_moe_final_candidate_selection/report.md) 把选择范围扩展到全部九个 same-shape Qwen3 MoE candidates，并和两个 source endpoint 做同场比较。规则是：source endpoints 必须先通过 eval-bundle audit；candidate 必须通过 vLLM bundle audit 和 checkpoint audit；如果被任一 source 在可用 score 上支配、某个任务跌破两个 source、Wilson score confidence gate 下仍可能低于 source 下界，或在同一批 prediction 样本上以 exact paired sign-test 显著净丢给 source，就不能选。当前真实状态是 `awaiting_source_eval`、`0/9` candidate usable；[selector smoke](results/qwen3_moe_final_candidate_selection_smoke/report.md) 覆盖 candidate-win、source-dominance、task-regression、small-sample uncertainty、significant paired-prediction regression、noisy paired delta 和 partial/provisional selection，`7/7` 通过。

最新 [Qwen3 MoE vLLM Eval Bundle Audit](results/qwen3_moe_eval_bundle_audit/report.md) 又在 selector 前加了一层结果包校验：每个 source/candidate 的 `summary.json`、`eval_plan.csv`、`metrics.csv`、`model_summary.csv` 和 `predictions.csv` 必须存在，模型名必须和 gate 计划一致，四个任务必须齐全，每任务样本数不能低于计划值，且每个任务都有 primary score；此外还检查 `task_manifest_sha256` 和 paired prediction key，防止不同 candidate 偷偷用了不同样本或不可配对预测。当前真实状态是 `awaiting_eval`、`0/11` 个 eval bundle 可用于选择；[audit smoke](results/qwen3_moe_eval_bundle_audit_smoke/report.md) 覆盖 valid、stale-model、missing-task、low-example、key-mismatch 和 manifest-mismatch 六种情况，`6/6` 通过。

最新 [Qwen3 MoE Mechanism Effect Attribution](results/qwen3_moe_mechanism_effect_attribution/report.md) 把 source frontier -> route-guarded -> audit-gated -> trust-region -> expert-only -> tail-trimmed -> searched cap-law -> layer/chunk -> unified mechanism -> subspace-scaled ablation 拆成相邻机制对比。它现在只输出 structural delta，因为 vLLM eval bundle 还没通过 audit；真实状态是 `awaiting_eval`、`0/9` 个 transition 已评分。当前结构链条显示：audit-gated 消掉 routed `>0.75` 的 `675` 个 tensor，trust-region 再消掉 `150` 个，expert-only 清零 `288` 个 attention changed tensors，tail-trimmed 再消掉剩余 `14` 个 routed `>0.75` tensor，geometry-informed layer/chunk 相比 searched cap-law 又把 strict `>0.65` 从 `245` 降到 `89`，unified mechanism 再把 `>0.65` 从 `89` 降到 `0`，subspace-scaled ablation 则专门等待验证那 `17` 个局部子空间冲突 expert 是否有行为收益。后续一旦 vLLM 分数落盘，它会自动给出每一步对 avg/worst/task score 的影响，而不是只给最终排名。

最新 [Qwen3 MoE Post-Eval Refresh](results/qwen3_moe_post_eval_refresh/report.md) 把远端 vLLM eval 之后必须跑的步骤固定成一条命令：eval bundle audit、unified selector、final candidate selector、mechanism attribution、四个 smoke 和 `collect_results`。当前本地刷新链路 `9/9` 步通过，真实状态仍是 audit `0/11` usable、unified selector `awaiting_source_eval`、final selector `awaiting_source_eval`、attribution `0/9` scored；[plan-only report](results/qwen3_moe_post_eval_refresh_plan/report.md) 也写出了远端可复制的命令清单。
