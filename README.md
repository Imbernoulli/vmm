...
27. **真实 Qwen MoE 已从 gate 推进到本地 checkpoint。** 旧的 [MoE Materialization Pipeline Plan](results/moe_materialization_pipeline_plan/report.md) 负责说明 gate 结构；当前真正进展见 [Checkpoint Materialization Readiness](results/checkpoint_materialization_readiness/report.md)：`qwen3_moe_unified_route_guarded_candidate` 已经从 writer dry-run 推进到 `materialized_checkpoint_exists`，输出目录有 `16` 个 shard、`18,867` 个同构 tensor，下一步是 vLLM 下游评测。
