...
最新 [Qwen3 MoE Adaptive Eval Schedule](results/qwen3_moe_adaptive_eval_schedule/report.md) 不再只靠手写候选顺序排队，而是把 mechanism coverage 和真实 safetensors delta frontier 合成调度优先级：source controls 仍必须先跑，第一轮候选 probe 仍是 `5` 个、覆盖 `5` 个机制问题；结构分数最高的是 `qwen3_moe_mechanistic_unified_candidate`，score `0.993`，原因是 total relative delta `0.238226`、routed relative delta `0.246441`、max routed tensor `0.649063`、routed `>0.65` 为 `0` 且 attention/router freeze。`subspace_scaled` 结构分数也高 (`0.985`)，但因为它只回答 localized subspace ablation，当前会等 source controls 和第一轮更广机制 probe 之后再决定是否扩展。
