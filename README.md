...
最新 [Qwen3 MoE vLLM Eval Budget Plan](results/qwen3_moe_eval_budget_plan/report.md) 把“正当评测要跑多少”也落成脚本：原 gate 的 `64` examples 只保留为 audit floor，预算版 `run_eval_budget.sh` 会把 ready-to-host 的十个 source/candidate 的 `--max-examples` 提到 `384`；同时预算表把 router calibration 的 active `cap001` 和 `margin_profile` 纳入 planned queue，但因为 checkpoint 尚未物化，runner 现在会跳过它们。这个数来自两个约束：binary task score 的 95% Wilson half-width 要压到 `0.05` 以内需要 `381` 条，四舍五入到 `384`；paired prediction gate 若要在 alpha `0.05` 下看出约 `5pp` 的净 source advantage，按 `25%` discordance 估计需要约 `248` 条 shared examples。新的 planned prompt budget 是 `18,432`，其中当前可直接 host 的 ready budget 是 `15,360`；active/ready/pending/plan-pruned router caps 是 `2/0/2/2`。因此下一轮远端 vLLM 不是静态 smoke，也不会默认浪费在 margin gate 已拒绝的 `cap0025/cap005` 上，而是能支撑 source dominance、task regression、score-confidence 和 paired-prediction regression 的机制评测；HumanEval 受 `164` 条数据上限限制，selector 会按实际样本数计算区间。
