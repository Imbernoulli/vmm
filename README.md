...
最新 [Qwen3 MoE Post-Eval Refresh](results/qwen3_moe_post_eval_refresh/report.md) 把远端 vLLM eval 之后必须跑的步骤固定成一条命令：candidate trust-region gate、eval budget planner、eval bundle audit、unified/final selector、mechanism attribution、feedback/mechanistic optimizer、mechanistic evidence/sensitivity、router-expert coupling、router-coupled candidate/frontier、source-set complementarity、average source-set optimizer、Qwen source discovery/eval/preflight、source-frontier eval feedback、router calibration frontier、HARC router stats、HARC router solver、HARC readiness gate、unified average optimizer、average method gate matrix、average trust-region bounds、mechanism leverage map、十四个 smoke 和 `collect_results`。当前本地刷新链路 `42/42` 步通过，真实状态仍是 audit `0/13` usable、unified selector `awaiting_source_eval`、final selector `awaiting_source_eval`、attribution `0/10` scored；source-frontier eval feedback 仍在等待远端 vLLM metrics（`0/4` jobs scored、final candidates `0`），但新增 [Qwen Source Frontier Eval Feedback Smoke](results/qwen_source_frontier_eval_feedback_smoke/report.md) 已验证反馈 gate 的三种机制分支：`3/3` jobs scored，positive-surplus frontier -> `final_average_budget_candidate`，low-surplus frontier -> `probe_only_below_interference_budget`，source-dominated frontier -> `source_frontier_not_better_than_endpoint`。HARC router stats 当前是 `harc_router_stats_missing_cache`：真实 cache `0/0` routers、first-stage `0/15`，stats smoke `6/6` checks 通过；HARC router solver 当前是 `harc_solver_missing_cache`，solver smoke `6/6` checks 通过并把 synthetic route KL `0.031806 -> 0.000006`；HARC readiness 当前是 `harc_ready_for_curvature_collection_waiting_cache`：`6/6` preconditions 通过、cache 缺失、top priority layer 是 `L17`、第一阶段 `15` 层，readiness smoke `4/4` 通过。这把 router 平均的下一步限定为先采集 Hessian/covariance 统计，再跑 matrix-free HARC solver，而不是直接接受 router weight average。当前 source-set surplus optimizer 的 top `coder+thinking` 仍只有 `0.0083` frontier gain，低于 `0.0694` observed merge interference budget，surplus `-0.0611`，所以只能 probe-only；source discovery eval plan 已生成 `4` 个 vLLM jobs、`8` 个 manifest prepare commands 和 `run_source_frontier_eval.sh`，served-model preflight 当前是 `static_preflight_ready_waiting_for_endpoint_model_list`，远端带 `--base-url` 后会直接用 `/models` 检查缺失模型。unified average optimizer 仍是 `built_waiting_for_qwen3_vllm_eval`，下一实验命令是 `results/qwen3_moe_eval_budget_plan/run_eval_budget.sh final`；[plan-only report](results/qwen3_moe_post_eval_refresh_plan/report.md) 写出了可复制的远端命令清单。
