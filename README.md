...
> **最新（机制驱动 unified average，2026-06）：** 见 **[Unified Average Optimizer](results/unified_average_optimizer/report.md)**、**[Qwen3 MoE Source-Set Complementarity Gate](results/qwen3_source_set_complementarity_gate/report.md)**、**[Qwen3 Average Source-Set Optimizer](results/qwen3_average_source_set_optimizer/report.md)**、**[Qwen Source Discovery Plan](results/qwen_source_discovery_plan/report.md)**、**[Qwen Source Discovery vLLM Eval Plan](results/qwen_source_discovery_eval_plan/report.md)**、**[Qwen Source Discovery Served-Model Preflight](results/qwen_source_discovery_served_model_preflight/report.md)**、**[Qwen Source Frontier Eval Feedback](results/qwen_source_frontier_eval_feedback/report.md)**、**[Qwen3 MoE vLLM Eval Budget Plan](results/qwen3_moe_eval_budget_plan/report.md)**、**[Qwen3 MoE Expert Geometry Probe](results/qwen3_moe_expert_geometry_probe/report.md)**、**[Qwen3 MoE Router Calibration NLL Probe](results/qwen3_moe_router_calibration_nll_probe/report.md)**、**[Qwen3 MoE Router Calibration Frontier](results/qwen3_moe_router_calibration_frontier/report.md)**、**[Qwen3 MoE Downstream Confidence Audit](results/fp_downstream_confidence_audit/report.md)** 和 **[Qwen3 MoE Unified Mechanism Candidate](results/qwen3_moe_unified_mechanism_candidate/report.md)**。当前 unified 规则已经从 route/geometry-only 更新为 subspace-aware：Dense 仍拒绝默认 midpoint；MoE 先固定 expert identity、冻结 direct router movement，再用 route/evidence/geometry/subspace cap 生成 same-shape expert rules。Qwen3 Instruct/Coder 直线 connectivity 仍不支配 endpoint，router-only NLL probe 说明 dispatch calibration 是真实杠杆但还不是接受条件；新的 router calibration frontier 把这个杠杆收进 HARC/Router-KD-style 的 margin-safe frontier：`cap001` 和 `margin_profile` 是默认 probe，`cap0025/cap005` 只能作为 stress ablation，下游生成置信审计也显示 router-cal 的 confident source-frontier wins 是 `0/3`。新增 source-set gate 把“平均前先看 source 是否互补”落成硬条件：当前 `instruct+coder` 被判为 `source_dominated_not_averageable_as_final`，dominant source 是 `instruct`，frontier avg gain 是 `0.000`，最好的 observed router-cal merge 仍比 source frontier 低 `0.069`。新增 source-set surplus optimizer 进一步把“有互补”拆成“互补是否足够覆盖已观测 merge 干扰”：当前 top source set 是 `coder+thinking`，source weights 是 `{"coder": 0.6667, "thinking": 0.3333}`，frontier avg gain 只有 `0.0083`，低于 observed merge interference budget `0.0694`，surplus `-0.0611`，所以只能进入 probe-only/endpoint 扩样，不能拿 final-average budget。现在这个判断又被拆到 task level：`0/3` 个任务有正 surplus，`2/3` 个任务没有 frontier gain，唯一正 gain 是 `mmlu +0.0250`，仍低于 interference budget；这说明 `coder+thinking` 只是弱互补，不是足够强的 average source set。新的 source discovery plan 把这个负 surplus 变成下一批实验队列；eval plan 进一步落成 `4` 个 vLLM source-frontier jobs，并修正 runner 任务名为 `humaneval_compile`，避免计划写成 runner 不会执行的 `humaneval`；served-model preflight 现在会在真正跑 downstream 前检查计划里的 model id 是否出现在 vLLM `/models`；source frontier eval feedback 会在远端 metrics 落盘后自动计算 task-wise source frontier、best single endpoint 和 `source_frontier_avg_gain - observed_merge_interference_budget`，当前本地状态仍是等待远端结果。最高优先 measured action 是扩样 `coder+thinking` endpoint eval，registry 里的最高优先源集合是 `dense_7b_general_code_math_reasoning`，随后是 `moe_30b_general_code_route_aware`；二者都必须先证明 `source_frontier_gain - observed_merge_interference_budget >= 0`，再进入同构 average。新的 optimizer contract 把这些判断落成硬 gate：`13` 个 unified-average 必要条件里 `11` 个已经通过，新增通过项包括 router-coupled retention frontier gate、source-set complementarity gate 和 source-set surplus budget gate；剩下 `downstream_source_dominance_gate` 和 `final_unified_average_acceptance` 都明确卡在同一 task manifest 的 vLLM eval。因此当前可交付结果是一个已物化、已 delta-audit 的 `subspace_cap_s1.00` 同构 checkpoint，最终是否比 source endpoints 好，仍必须由同一 task manifest 下的 vLLM eval 决定。

> **HARC stats/solver/candidate/readiness 新增：** [Qwen3 MoE HARC Router Stats](results/qwen3_moe_harc_router_stats/report.md) 已把 HARC-style router 目标需要的二阶量落成脚本：从 router calibration cache 读取每层 hidden states 和 teacher logits，计算 softmax Hessian trace/diag/offdiag、top-k boundary margin、hidden covariance trace/stable-rank 和 matrix-free solver input。当前本地真实状态是 `harc_router_stats_missing_cache`，因为 `results/qwen3_moe_router_calibration_cache/router_calibration_cache.pt` 还没由 GPU host 采集；第一阶段缺的仍是 `15` 个高优先 router layers。synthetic [stats smoke](results/qwen3_moe_harc_router_stats_smoke/report.md) 已验证 ready 分支，`2/2` routers、first-stage `2/2`、checks `6/6`，并覆盖 top-k boundary Hessian 正值。[HARC Router Delta Solver](results/qwen3_moe_harc_router_solver/report.md) 把二阶目标继续推进到同构 router delta：真实路径当前是 `harc_solver_missing_cache`，但 [solver smoke](results/qwen3_moe_harc_router_solver_smoke/report.md) 已验证 matrix-free CG 解，`2` 个 synthetic router 写出 `router_delta.safetensors`，route KL `0.031806 -> 0.000006`，quadratic proxy 同步下降，checks `6/6`。新增 [HARC Router Candidate](results/qwen3_moe_harc_router_candidate/report.md) 把 solver 输出接到真实 checkpoint 写出 gate：真实状态是 `harc_router_candidate_waiting_for_solver_delta`，不会在 cache/delta 缺失时伪造 checkpoint；[candidate smoke](results/qwen3_moe_harc_router_candidate_smoke/report.md) 已实际写出 tiny same-shape checkpoint，验证 `2/2` router tensors 精确等于 `base + delta`、`1/1` 非 router control tensor 不变，checks `3/3`、max error `0.0`。在此基础上，[Qwen3 MoE HARC Readiness Gate](results/qwen3_moe_harc_readiness_gate/report.md) 现在不再用“目录存在”判断 cache，而是读取 stats summary；direct router average 已被拒绝、top-k boundary fragility 已检测、router-only repair 有正 NLL 信号、router-expert coupling active、margin-safe calibration frontier 和 calibration job preflight 都已就绪，`6/6` preconditions 通过，但 Hessian/covariance cache 仍缺失，所以状态保持 `harc_ready_for_curvature_collection_waiting_cache`。当前优先层是 `L17`，HARC priority `0.7379`；readiness [smoke](results/qwen3_moe_harc_readiness_gate_smoke/report.md) 覆盖 waiting-cache、solver-ready、router-safe-no-HARC、no-local-repair 四种分支，`4/4` 通过。
...
最新 [Qwen3 MoE Mechanism-Gated vLLM Eval Gate](results/qwen3_moe_mechanism_eval_gate/report.md) 已把这个问题转成可执行评测：两个 source endpoint 加 `10` 个已物化 same-shape Qwen3 MoE candidates 是 `ready_to_host`，`qwen3_moe_router_coupled_candidate` 作为 `checkpoint_missing_until_materialized` 的机制消融排队，`qwen3_moe_harc_router_candidate` 作为 `checkpoint_missing_until_harc_solver_delta` 的二阶 router calibration 消融排队。这个 gate 不再问“哪个算法名最好”，而是逐项检验 tail delta cap、route/load trust-region、shared attention ablation、0.65 tail trim、risk penalty simplification、layer/chunk sensitivity、unified mechanism optimizer、subspace conflict ablation、router-coupled boundary ablation、HARC router calibration ablation 和 endpoint fallback；如果所有 average 候选被 source 支配，selection rule 会返回同构 endpoint/no-average。当前本机 `nvidia-smi` 不可用，所以 selection 状态是 `awaiting_remote_vllm_eval`。

最新 [Qwen3 MoE vLLM Eval Budget Plan](results/qwen3_moe_eval_budget_plan/report.md) 把“正当评测要跑多少”和“先跑哪些”都落成脚本：原 gate 的 `64` examples 只保留为 audit floor，预算版 `run_eval_budget.sh` 会把 `--max-examples` 提到 `384`；但默认请求已经从全量 ablation 改成 `final`，只跑两个 source endpoint 加两个 candidate trust-region gate 允许进入最终选择的候选：`qwen3_moe_mechanistic_unified_candidate` 和 `qwen3_moe_subspace_scaled_candidate`。这 4 个 final-core 方法的 recommended prompt budget 是 `6,144`；另外 `10` 个候选留在 `mechanism` 队列，recommended prompt budget `15,360`，其中 `8` 个已 ready-to-host，`qwen3_moe_router_coupled_candidate` 需要先物化和 delta audit，`qwen3_moe_harc_router_candidate` 还在等真实 HARC solver delta；router calibration 的 active `cap001`/`margin_profile` 仍是 `2` 个 pending materialization。`384` 这个数来自两个约束：binary task score 的 95% Wilson half-width 要压到 `0.05` 以内需要 `381` 条，四舍五入到 `384`；paired prediction gate 若要在 alpha `0.05` 下看出约 `5pp` 的净 source advantage，按 `25%` discordance 估计需要约 `248` 条 shared examples。因此下一轮远端 vLLM 不是静态 smoke，也不会默认把 GPU 浪费在已经被 final selector 排除的 ablation-only 候选上；HumanEval 受 `164` 条数据上限限制，selector 会按实际落盘样本数计算区间。
...
最新 [Qwen3 MoE Post-Eval Refresh](results/qwen3_moe_post_eval_refresh/report.md) 把远端 vLLM eval 之后必须跑的步骤固定成一条命令：candidate trust-region gate、eval budget planner、eval bundle audit、unified/final selector、mechanism attribution、feedback/mechanistic optimizer、mechanistic evidence/sensitivity、router-expert coupling、router-coupled candidate/frontier、source-set complementarity、average source-set optimizer、Qwen source discovery/eval/preflight、source-frontier eval feedback、router calibration frontier、HARC router stats、HARC router solver、HARC router candidate、HARC readiness gate、unified average optimizer、average method gate matrix、average trust-region bounds、mechanism leverage map、十五个 smoke 和 `collect_results`。当前本地刷新链路 `44/44` 步通过，真实状态仍是 audit `0/14` usable、unified selector `awaiting_source_eval`、final selector `awaiting_source_eval`、attribution `0/10` scored；source-frontier eval feedback 仍在等待远端 vLLM metrics（`0/4` jobs scored、final candidates `0`），但 [Qwen Source Frontier Eval Feedback Smoke](results/qwen_source_frontier_eval_feedback_smoke/report.md) 已验证反馈 gate 的三种机制分支：`3/3` jobs scored，positive-surplus frontier -> `final_average_budget_candidate`，low-surplus frontier -> `probe_only_below_interference_budget`，source-dominated frontier -> `source_frontier_not_better_than_endpoint`。HARC router stats 当前是 `harc_router_stats_missing_cache`：真实 cache `0/0` routers、first-stage `0/15`，stats smoke `6/6` checks 通过；HARC router solver 当前是 `harc_solver_missing_cache`，solver smoke `6/6` checks 通过并把 synthetic route KL `0.031806 -> 0.000006`；HARC router candidate 当前是 `harc_router_candidate_waiting_for_solver_delta`，真实 checkpoint 尚未写出，原因是缺少 solver delta；candidate smoke 已写出 tiny checkpoint，验证 `2/2` router delta 生效、`1/1` 非 router tensor 不变、checks `3/3`。HARC readiness 当前是 `harc_ready_for_curvature_collection_waiting_cache`：`6/6` preconditions 通过、cache 缺失、top priority layer 是 `L17`、第一阶段 `15` 层，readiness smoke `4/4` 通过。这把 router 平均的下一步限定为先采集 Hessian/covariance 统计，再跑 matrix-free HARC solver，最后 materialize router-only same-shape candidate 进入 matched vLLM gate，而不是直接接受 router weight average。当前 source-set surplus optimizer 的 top `coder+thinking` 仍只有 `0.0083` frontier gain，低于 `0.0694` observed merge interference budget，surplus `-0.0611`，所以只能 probe-only；source discovery eval plan 已生成 `4` 个 vLLM jobs、`8` 个 manifest prepare commands 和 `run_source_frontier_eval.sh`，served-model preflight 当前是 `static_preflight_ready_waiting_for_endpoint_model_list`，远端带 `--base-url` 后会直接用 `/models` 检查缺失模型。unified average optimizer 仍是 `built_waiting_for_qwen3_vllm_eval`，下一实验命令是 `results/qwen3_moe_eval_budget_plan/run_eval_budget.sh final`；[plan-only report](results/qwen3_moe_post_eval_refresh_plan/report.md) 写出了可复制的远端命令清单。
