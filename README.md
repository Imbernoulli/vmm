...
11. **同构 checkpoint 写出路径已经打通。** `scripts/write_same_shape_average_checkpoint.py` 会先验证 tensor name/shape，再按 `base + sum_i w_i * (source_i - base)` 写 safetensors；现在支持 `--tensor-add-csv` 写 router-bias scalar delta，也支持 `--packed-expert-rule-csv` 对 Qwen-style packed expert tensor 的第 0 维做 slice-level source expert remap/weight。Qwen2.5-0.5B base/instruct/coder dry-run 已检查 `290` 个 tensor 无缺失、无 shape mismatch；本地还已写出 `qwen_0_5b_instruct_coder_uniform_average` 这个 0.5/0.5 Dense 负 baseline checkpoint，用于验证真实 host/eval 管线。MoE writer smoke 现在同时覆盖 freeze-router、bias additive correction、展开式 expert alias 和 packed expert slice rule。
