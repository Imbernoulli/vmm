...
27. **真实 Qwen MoE 已从 gate 推进到两个本地 checkpoint 和文件级 delta audit。** 当前真正进展见 [Checkpoint Materialization Readiness](results/checkpoint_materialization_readiness/report.md)、[Materialized Checkpoint Delta Audit](results/qwen3_moe_materialized_delta_audit/report.md)、[Qwen3 MoE Audit-Gated Candidate](results/qwen3_moe_audit_gated_candidate/report.md) 和 [audit-gated delta audit](results/qwen3_moe_audit_gated_delta_audit/report.md)：`qwen3_moe_unified_route_guarded_candidate` 和 `qwen3_moe_audit_gated_candidate` 都已物化成本地 `16` shard / `18,867` tensor 同构 checkpoint。audit-gated candidate 把 `302/5,243` 个高 relative-delta expert rules 缩小后，routed expert 单 tensor max relative delta 从 `1.327` 降到 `0.750`，`>1.0` 的 routed tensors 从 `182` 降到 `0`，router 仍是 `0/48` 改动。下一步是 vLLM 下游评测。
...
32. **Checkpoint materialization readiness 已推进到两个 Qwen3 candidates ready for eval。** [Checkpoint Materialization Readiness](results/checkpoint_materialization_readiness/report.md) 现在显示 `9` 个候选里 `3` 个已 materialize，其中 Qwen3 route-guarded candidate 和 Qwen3 audit-gated candidate 都是 `ready_for_vllm_eval`，0.5B uniform baseline 已完成 vLLM eval；[vLLM Checkpoint Eval Plan](results/vllm_checkpoint_eval_plan/report.md) 显示 `8` 个 eval rows，其中 Qwen3 Instruct source、Qwen3 Coder source、两个 Qwen3 MoE candidates 和 0.5B baseline 都是 `ready_to_host`，`2` 个缺 checkpoint，`1` 个 toy 不可加载。
