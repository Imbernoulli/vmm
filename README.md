...
> **最新（机制驱动 unified average，2026-06）：** 见 **[Unified Average Optimizer](results/unified_average_optimizer/report.md)**、**[Qwen3 MoE vLLM Eval Budget Plan](results/qwen3_moe_eval_budget_plan/report.md)**、**[Qwen3 MoE Mechanism Leverage Map](results/qwen3_moe_mechanism_levers/report.md)** 和 **[Qwen3 MoE Layer/Chunk Coefficient Candidate](results/qwen3_moe_layer_chunk_candidate/report.md)**。这个脚本把 Dense barrier、MoE expert gauge、Qwen3 expert identity、router movement 和最终 vLLM 选择 gate 合成一个同构操作策略，而不是静态判断哪个算法名最好。当前证据是：(1) Dense midpoint 的二阶 Fisher 预测严重失效，general/code actual-predicted ratio 为 `42.86/26.66`，所以拒绝默认 `0.5/0.5`；(2) 真实 OLMoE gauge 反事实中，同名 average 让 NLL 从 `4.1678` 升到 `9.6588`，恢复 expert 对齐后回到 `4.1678`；(3) Qwen3-30B-A3B Instruct vs Coder 的 expert identity 通过，`48/48` 层 identity-optimal，但 router direct move gate 为 `0/48` 层通过，所以先 freeze/router-KD gate；(4) 八个 Qwen3 same-shape candidates 仍需同场 vLLM eval，当前 final selector 是 `awaiting_source_eval`、`0/8` eligible；(5) Qwen3 MoE 远端 eval 不再按 `64` 条 smoke 样本硬选，新的预算脚本按 Wilson 95% half-width `0.05` 和 paired sign-test alpha `0.05` 推荐 `384` max examples，10 个 source/candidate 总预算 `15,360` prompts；(6) 机制优先级现在明确：先跑预算化 downstream eval，其次拒绝 direct router movement，再检验 routed expert tail cap、risk penalty simplification、shared-attention ablation 和 layer/chunk 系数。
...
最新 [Qwen3 MoE Mechanism Leverage Map](results/qwen3_moe_mechanism_levers/report.md) 把现有 probe 结果转成“下一步该优化什么”的优先级：`source_and_candidate_downstream_eval` 排第一（`0.98`），因为没有同场 vLLM 分数就不能接受 average；`router_direct_movement` 排第二（`0.94`），因为 router move gate 是 `0/48` 层允许、min top1 agreement 只有 `0.069`；`routed_expert_tail_cap_0_75`、`risk_penalty_complexity`、`tail_cap_0_65` 和 `route_load_trust_region` 依次排在后面。它还给出一个 importance-guided layer/chunk calibration plan：高敏感层 `12,17,18,20,21,22,23,26` 以后应优先学习 per-layer coefficients，而低敏感层共享粗粒度 coefficients。这对应近期 MoE merging/Expert Merging 里“层和 expert 敏感性不均匀”的机制，但仍保持 same-shape writer 约束。

最新 [Qwen3 MoE Layer/Chunk Coefficient Candidate](results/qwen3_moe_layer_chunk_candidate/report.md) 已经把上面的 layer/chunk 机制判断落成同构 writer rules，而不是停在文档层面。选中的 schedule 是 `policy_095_098_100`：高敏感层 Coder contribution 乘 `0.95`，中等敏感 two-layer chunks 乘 `0.98`，低敏感层保持 `1.0`；它在 retention `>=0.975` 和 max relative delta `<=0.65` 约束下，保留 route-mass weighted Coder contribution `0.9851`，风险加权 delta proxy 降低 `0.0207`。现在它已经物化成真实 same-shape checkpoint，写出 `16` 个 safetensors shards，delta audit 通过：`18,867` tensors 同构、`10,353` 个 routed expert FFN tensors 改动、attention/norm/embedding/router 全部不动，total relative delta norm `0.243`，routed max relative delta `0.65003`。这个结果说明 layer/chunk sensitivity 已进入真实候选集合；它能不能优于 source 或 searched cap-law 仍必须由同场 vLLM 决定。
...
当前 coverage audit：`complete = 78`, `partial = 1`, `missing = 0`；唯一 partial 是 generic target-registry vLLM eval 还没有跑完，但 materialized checkpoint、source-vs-merge 对照、probe-guided dense candidate、dense guard ablation 的真实 vLLM eval、真实 Qwen3 MoE materialized checkpoint，以及 probe-gated/unified average selection gate 都已完成。完整汇总见 `results/summary.md` 和 `results/summary.json`。
