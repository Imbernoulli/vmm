...
最新 dense sparse-method candidate 结果：新的 `qwen_0_5b_sparse_method_bridge` 不再问“哪个场景套哪个算法”，而是把机制拆到坐标级。它沿用 global bridge 的 `instruct=0.25,coder=1.0`，再从 Qwen instruct/coder 的 tensor conflict probe 中选出 sign conflict `>=0.44`、delta cosine `<=0.16`、参数量 `>=100k` 的 attention/MLP tensor，对这些 tensor 执行 TIES-style coordinate trim/sign-elect/merge。当前选中 `99` 个 tensor、覆盖约 `48.73%` 参数；same-shape writer dry-run 已确认 `99` 条 `tensor_method:*:ties` 全部命中，其余 `191` 个 floating tensor 仍走 global bridge 线性规则。这个候选要检验的机理是：global coefficient 负责避开 midpoint ridge，高冲突坐标再用 sparse task-vector rule 消解符号冲突；它还没跑真实 vLLM eval，所以现在是 materialization-ready candidate，而不是已证实优于 global bridge 的最终结果。
...
26. **Sparse-coordinate candidate 是下一步 unified dense 实验。** [Qwen Dense Sparse-Method Candidate](results/qwen_dense_sparse_method_candidate/report.md) 沿用 global bridge 的 `instruct=0.25,coder=1.0`，只对 sign conflict 高、delta cosine 低的 attention/MLP tensor 做 TIES coordinate merge；当前选中 `99` 个 tensor、覆盖约 `48.73%` 参数，same-shape dry-run 确认 `99` 条 sparse method rule 全部命中。这是要验证“global coefficient + coordinate conflict resolution”的统一机制，还需要 materialize 后跑真实 vLLM eval。
27. **真实 Qwen MoE materialization 现在有明确 gate，不再把 toy smoke 当成实证。** [MoE Materialization Pipeline Plan](results/moe_materialization_pipeline_plan/report.md) 把目标模型选择、真实 topology/header probe、routing probe、readiness、route-weight rules、layer-wise expert remap、router-bias delta、same-shape writer 和 vLLM eval 串成 10 个 gate；当前状态是 `waiting_for_real_moe_probe_or_paths`，blocking stage 是 `exact_moe_topology`，`3` 个 gate ready/complete、`7` 个 gate waiting/blocked。下一条真实 MoE probe 已指向 `Qwen/Qwen3-30B-A3B` vs `Qwen/Qwen3-Coder-30B-A3B-Instruct`，只有拿到真实 expert load / route overlap / layer-wise expert match 后，才会生成真实 Qwen MoE checkpoint writer command。
28. **Probe-gated unified average 已把“为什么”转成 action gate。** [Probe-Gated Unified Average Plan](results/probe_gated_unified_average_plan/report.md) 不是按场景静态排名方法，而是从已有 vLLM ablation 和 toy MoE 机制对比里提炼默认动作：Dense 侧只保留 `probe_guided_global_bridge_only`，因为 global bridge 比 uniform avg primary 高 `0.023`，而 aggressive module guard 低 `0.043`；MoE 侧默认组合是 expert identity alignment、confidence-blended expert weights、guarded router calibration 和 capacity gate，其中 expert identity matching 的 soft worst-acc gain 是 `0.205`，capacity bias 的 top-k overflow delta 是 `-0.029`。真实 Qwen MoE 仍必须先过 `exact_moe_topology` 和 routing probe。
29. **Checkpoint materialization readiness 已推进到 hosted eval complete。** [Checkpoint Materialization Readiness](results/checkpoint_materialization_readiness/report.md) 现在显示 7 个候选里 `1` 个已 materialize 且 `1` 个完成 vLLM eval，`4` 个仍被 placeholder source path 卡住；[vLLM Checkpoint Eval Plan](results/vllm_checkpoint_eval_plan/report.md) 显示 4 个 checkpoint 候选里 `1` 个 eval complete、`2` 个缺 materialized checkpoint、`1` 个 toy 不可加载。
...
当前 coverage audit：`complete = 47`, `partial = 1`, `missing = 0`；唯一 partial 是 generic target-registry vLLM eval 还没有跑完，但 materialized checkpoint、source-vs-merge 对照、probe-guided dense candidate、dense guard ablation 的真实 vLLM eval、真实 MoE materialization gate plan，以及 probe-gated unified average plan 都已完成。完整汇总见 `results/summary.md` 和 `results/summary.json`。
...
| Qwen dense sparse-coordinate candidate | 完成 | 99 个 high-conflict attention/MLP tensor 的 TIES sparse rule 已通过 same-shape writer dry-run |
