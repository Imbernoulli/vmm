...
新增 [Qwen3 MoE Downstream Generation Matrix](results/fp_downstream_matrix/report.md) 把机制判断放到真实生成任务上再看一次：官方 Qwen3 MoE parents 和平均候选在 MMLU/GSM8K/HumanEval 上评测后，best avg 仍是 `instruct` (`0.897`)，naive Instruct+Coder average 只有 `0.794`；只做 router calibration 后升到 `0.828`，其中 HumanEval `+0.075`、GSM8K `+0.025`。这证明 router calibration 确实能恢复一部分被平均糊掉的 dispatch，但它仍比 best parent avg 低 `0.069`，所以这里只把它作为“为什么 router 是 MoE 专属优化杠杆”的辅助证据，不替代最终同场 vLLM selector。
