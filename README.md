...
最新 Qwen3 MoE audit-gated candidate：基于上面的真实 delta audit，我新增 [Qwen3 MoE Audit-Gated Candidate](results/qwen3_moe_audit_gated_candidate/report.md)。它保留 route-frequency expert rule，但对实际 FFN relative delta norm 超过 `0.75` 的 expert 按比例缩小非 base source（Coder）delta weight；`5,243` 个 expert rules 中 `302` 个被缩小，mean effective nonbase weight 从 `0.201` 降到 `0.193`，最大 audited relative delta 原本是 `1.327`，最强缩放系数是 `0.565`。新 tensor rules 已通过真实 Qwen3 writer dry-run：`18,867` 个 floating tensors、`48` 个 router 冻结、`288` 个 attention rule 命中、expert rules 正常命中。这个候选是更保守的下一版，不替代 vLLM eval；它的作用是把“probe 发现少数 expert 移动过大”转成可物化的规则。
...
27. **真实 Qwen MoE 已从 gate 推进到本地 checkpoint、文件级 delta audit 和下一版 audit-gated 规则。** 当前真正进展见 [Checkpoint Materialization Readiness](results/checkpoint_materialization_readiness/report.md)、[Materialized Checkpoint Delta Audit](results/qwen3_moe_materialized_delta_audit/report.md) 和 [Qwen3 MoE Audit-Gated Candidate](results/qwen3_moe_audit_gated_candidate/report.md)：`qwen3_moe_unified_route_guarded_candidate` 已经从 writer dry-run 推进到 `materialized_checkpoint_exists`，输出目录有 `16` 个 shard、`18,867` 个同构 tensor；delta audit 证明 router `0/48` 改动、changed tensor `10,641`；audit-gated candidate 又把 `302/5,243` 个高 relative-delta expert rules 缩小，并通过真实 Qwen3 writer dry-run。下一步是 vLLM 下游评测。
...
32. **Checkpoint materialization readiness 已推进到 Qwen3 candidate ready for eval。** [Checkpoint Materialization Readiness](results/checkpoint_materialization_readiness/report.md) 现在显示 `9` 个候选里 `2` 个已 materialize，其中 Qwen3 route-guarded candidate 是 `ready_for_vllm_eval`，Qwen3 audit-gated candidate 是 `needs_materialization_after_dry_run`，0.5B uniform baseline 已完成 vLLM eval；[vLLM Checkpoint Eval Plan](results/vllm_checkpoint_eval_plan/report.md) 显示 `8` 个 eval rows，其中 Qwen3 Instruct source、Qwen3 Coder source、Qwen3 route-guarded candidate 和 0.5B baseline 都是 `ready_to_host`，`3` 个缺 checkpoint，`1` 个 toy 不可加载。
