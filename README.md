...
> **当前以本段为准（2026-06-20）：** Qwen3 MoE 的下一版同构 average 已经从“规则名选择”推进到可物化的 mechanistic scale law：[Qwen3 MoE Mechanistic Unified Candidate](results/qwen3_moe_mechanistic_unified_candidate/report.md) 以 `B/H/I = benefit / curvature / interference` 为每个 routed expert group 解非 base scale，选中 `s0.08_b1.65_h0.75_i0.75`，route-mass nonbase retention `0.9650`，max predicted routed relative delta `0.6500`，hard-cap violations `0`。它已经写成真实 checkpoint：`results/checkpoints/qwen3_moe_mechanistic_unified_candidate`，共 `16` 个 safetensors shard、约 `57G`；[materialized delta audit](results/qwen3_moe_mechanistic_unified_delta_audit/report.md) 直接读权重验证通过，total relative delta norm `0.238238`，routed expert FFN relative norm `0.246455`，attention `0/288` changed，router `0/48` changed，routed tensors `>0.65` 为 `20` 且 `>0.6505` 为 `0`。[mechanistic evidence audit](results/qwen3_moe_mechanistic_evidence_audit/report.md) 显示梯度/scale 方向一致率 `1.000`，局部 objective proxy 改善比例 `0.945`，主要压 scale 的内部信号是 `curvature_score`、`feature_router_instability`、`feature_expert_internal_geometry`。新的 [delta frontier](results/qwen3_moe_delta_frontier/report.md) 把 mechanistic candidate 纳入十个已 audit 候选，当前 best delta-safety candidate 是 `mechanistic_unified`；[vLLM checkpoint eval plan](results/vllm_checkpoint_eval_plan/report.md) 已把它标为 `ready_to_host`，评测集合对齐到 `14/14` methods。当前本机 `nvidia-smi` 失败，所以还没有本地 vLLM host/eval；下一步是用同一个 task manifest 跑 source endpoints 和 same-shape candidates 的 vLLM 下游评测。
