...
> **最新（机制驱动 unified average，2026-06）：** 见 **[Unified Average Optimizer](results/unified_average_optimizer/report.md)**、**[Qwen3 MoE vLLM Eval Budget Plan](results/qwen3_moe_eval_budget_plan/report.md)**、**[Qwen3 MoE Expert Geometry Probe](results/qwen3_moe_expert_geometry_probe/report.md)**、**[Qwen3 MoE Mechanism Leverage Map](results/qwen3_moe_mechanism_levers/report.md)** 和 **[Qwen3 MoE Layer/Chunk Coefficient Candidate](results/qwen3_moe_layer_chunk_candidate/report.md)**。这个脚本把 Dense barrier、MoE expert gauge、Qwen3 expert identity、router movement、真实专家内部几何和最终 vLLM 选择 gate 合成一个同构操作策略，而不是静态判断哪个算法名最好。当前证据是：(1) Dense midpoint 的二阶 Fisher 预测严重失效，general/code actual-predicted ratio 为 `42.86/26.66`，所以拒绝默认 `0.5/0.5`；(2) 真实 OLMoE gauge 反事实中，同名 average 让 NLL 从 `4.1678` 升到 `9.6588`，恢复 expert 对齐后回到 `4.1678`；(3) Qwen3-30B-A3B Instruct vs Coder 的 expert identity 通过，`48/48` 层 identity-optimal，但 router direct move gate 为 `0/48` 层通过，所以先 freeze/router-KD gate；(4) 新增真实 expert geometry probe，读取 `18,432` 个 routed expert projection tensor，mean/p05 cosine 为 `0.386/0.118`、mean/p95 relative delta 为 `1.062/1.275`，发现 `931` 个 high internal-geometry-risk experts 和 `204` 个 high route+geometry-risk experts；(5) 八个 Qwen3 same-shape candidates 仍需同场 vLLM eval，当前 final selector 是 `awaiting_source_eval`、`0/8` eligible；(6) Qwen3 MoE 远端 eval 不再按 `64` 条 smoke 样本硬选，新的预算脚本按 Wilson 95% half-width `0.05` 和 paired sign-test alpha `0.05` 推荐 `384` max examples，10 个 source/candidate 总预算 `15,360` prompts；(7) 机制优先级现在明确：先跑预算化 downstream eval，其次拒绝 direct router movement，再检验 routed expert tail cap、risk penalty simplification、shared-attention ablation 和 geometry-informed layer/chunk 系数。
...
当前最实在的 Qwen3 MoE 结果是：我已经把 route/load probe、category specialization、router fragility、真实 expert geometry 和真实 delta audit 合成一个同构 trust-region/checkpoint 候选链，并新增一个 [expert-only attention ablation](results/qwen3_moe_expert_only_trust_region_candidate/report.md) 去隔离 shared attention 的作用；随后又物化了 [tail-trimmed expert-only candidate](results/qwen3_moe_tail_trimmed_expert_only_candidate/report.md)，只对剩余 high-tail routed experts 做二次收缩。现在又把 cap-law search 里最简单的 `searched_no_gt065` 规则物化成 [searched cap-law candidate](results/qwen3_moe_searched_no_gt065_delta_audit/report.md)，并进一步物化了 geometry-informed [layer/chunk coefficient candidate](results/qwen3_moe_layer_chunk_delta_audit/report.md)：两者都冻结 router 和 shared attention，区别是后者把高敏感层 Coder contribution 继续按 layer/chunk 系数收缩。layer/chunk 的真实 safetensors audit 通过：attention `0/288` changed、router `0/48` changed、total relative delta norm `0.243`、routed `>0.75` 为 `0`、strict `>0.65` 为 `89`、`>0.6505` 为 `0`。现阶段不能提前说 layer/chunk 一定更好；真正的算法优劣还要看 Qwen3 Instruct/Coder source 和 8 个 same-shape candidates 在同一套下游任务上的表现。

最新 [Qwen3 MoE Delta Frontier Probe](results/qwen3_moe_delta_frontier/report.md) 把七个已物化 candidate 加一个 unified mechanism alias 的真实 safetensors delta 放到同一张表里。结论很明确：route-guarded -> audit-gated 主要消掉 routed expert 的危险大步长（`>1.0` 从 `182` 到 `0`，`>0.75` 从 `839` 到 `164`）；audit-gated -> trust-region 继续把 `>0.75` 从 `164` 压到 `14`；trust-region -> expert-only 不再改善 routed expert tail，只是把 attention delta 从 `0.189` 清零；expert-only -> tail-trimmed 才继续压 routed tail，把 `>0.65` 从 `366` 降到 `80`（剩余都在 `0.6505` rounding slop 内）；searched cap-law 则把复杂 penalty 换成统一 `0.65` cap，total relative norm `0.248`，strict `>0.65` tensor 为 `245`，但 `>0.6505` 为 `0`；geometry-informed layer/chunk 在 searched cap-law 基础上继续把 total relative norm 降到 `0.243`，strict `>0.65` 降到 `89`，`>0.6505` 仍为 `0`；当前 unified mechanism candidate 仍等价于 searched no-gt-0.65 规则。下一版 unified MoE 规则里，trust-region/tail-trim 是安全机制，attention、risk penalty 和 layer/chunk sensitivity 是否保留都必须交给同任务 vLLM eval 决定。

最新 [Qwen3 MoE Mechanism-Gated vLLM Eval Gate](results/qwen3_moe_mechanism_eval_gate/report.md) 已把这个问题转成可执行评测：两个 source endpoint 加八个 same-shape Qwen3 MoE candidates 都是 `ready_to_host`，并生成 `run_eval_gate.sh` 逐个 `vllm serve -> eval -> stop`。这个 gate 不再问“哪个算法名最好”，而是逐项检验 tail delta cap、route/load trust-region、shared attention ablation、0.65 tail trim、risk penalty simplification、layer/chunk sensitivity、unified mechanism alias 和 endpoint fallback；如果所有 average 候选被 source 支配，selection rule 会返回同构 endpoint/no-average。当前本机 `nvidia-smi` 不可用，所以 selection 状态是 `awaiting_remote_vllm_eval`。

最新 [Qwen3 MoE vLLM Eval Budget Plan](results/qwen3_moe_eval_budget_plan/report.md) 把“正当评测要跑多少”也落成脚本：原 gate 的 `64` examples 只保留为 audit floor，预算版 `run_eval_budget.sh` 会把十个 source/candidate 的 `--max-examples` 提到 `384`。这个数来自两个约束：binary task score 的 95% Wilson half-width 要压到 `0.05` 以内需要 `381` 条，四舍五入到 `384`；paired prediction gate 若要在 alpha `0.05` 下看出约 `5pp` 的净 source advantage，按 `25%` discordance 估计需要约 `248` 条 shared examples。新的总 prompt budget 是 `15,360`，比原 smoke gate 多 `12,800`。因此下一轮远端 vLLM 不是静态 smoke，而是能支撑 source dominance、task regression、score-confidence 和 paired-prediction regression 的机制评测；HumanEval 受 `164` 条数据上限限制，selector 会按实际样本数计算区间。
...
32. **Checkpoint materialization readiness 已推进到六个 Qwen3 checkpoint candidate 和一个 unified alias 可评测。** [Checkpoint Materialization Readiness](results/checkpoint_materialization_readiness/report.md) 现在显示 `12` 个候选里 `6` 个已 materialize，其中 Qwen3 route-guarded、audit-gated、trust-region、expert-only、tail-trimmed 和 searched no-gt-0.65 candidates 都是 `ready_for_vllm_eval`，0.5B uniform baseline 已完成 vLLM eval；[vLLM Checkpoint Eval Plan](results/vllm_checkpoint_eval_plan/report.md) 显示 Qwen3 Instruct source、Qwen3 Coder source、六个已物化 Qwen3 MoE candidates、unified mechanism alias 和 0.5B baseline 都是 `ready_to_host`。最新 [Unified Result Selector](results/qwen3_moe_unified_result_selection/report.md) 仍是 `awaiting_source_eval`，所以现在不会提前宣布 average 胜出。
...
当前 coverage audit：`complete = 79`, `partial = 1`, `missing = 0`；唯一 partial 是 generic target-registry vLLM eval 还没有跑完，但 materialized checkpoint、source-vs-merge 对照、probe-guided dense candidate、dense guard ablation 的真实 vLLM eval、真实 Qwen3 MoE materialized checkpoint、expert geometry probe，以及 probe-gated/unified average selection gate 都已完成。完整汇总见 `results/summary.md` 和 `results/summary.json`。
