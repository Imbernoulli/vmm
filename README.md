...
27. **真实 Qwen MoE 已从 gate 推进到两个本地 checkpoint、文件级 delta audit 和第三版 trust-region 规则。** 当前真正进展见 [Checkpoint Materialization Readiness](results/checkpoint_materialization_readiness/report.md)、[Materialized Checkpoint Delta Audit](results/qwen3_moe_materialized_delta_audit/report.md)、[Qwen3 MoE Audit-Gated Candidate](results/qwen3_moe_audit_gated_candidate/report.md)、[audit-gated delta audit](results/qwen3_moe_audit_gated_delta_audit/report.md) 和 [Qwen3 MoE Trust-Region Candidate](results/qwen3_moe_trust_region_candidate/report.md)：`qwen3_moe_unified_route_guarded_candidate` 和 `qwen3_moe_audit_gated_candidate` 都已物化成本地 `16` shard / `18,867` tensor 同构 checkpoint。trust-region candidate 进一步把 routing/load/category/delta probe 联合成 expert-level cap，缩小 `405/5,243` 个 expert rules，其中 `103` 个来自 MoE 风险而不是单纯 delta 阈值；writer dry-run 已命中 `15,729` 个 expert tensors、`288` 个 attention tensors 和 `48` 个 router freeze。下一步是物化 trust-region candidate 和 vLLM 下游评测。
...
32. **Checkpoint materialization readiness 已推进到两个 Qwen3 candidates ready for eval，第三个 trust-region candidate ready to materialize。** [Checkpoint Materialization Readiness](results/checkpoint_materialization_readiness/report.md) 现在显示 `10` 个候选里 `3` 个已 materialize，其中 Qwen3 route-guarded candidate 和 Qwen3 audit-gated candidate 都是 `ready_for_vllm_eval`，Qwen3 trust-region candidate 是 `needs_checkpoint_materialization`，0.5B uniform baseline 已完成 vLLM eval；[vLLM Checkpoint Eval Plan](results/vllm_checkpoint_eval_plan/report.md) 显示 `9` 个 eval rows，其中 Qwen3 Instruct source、Qwen3 Coder source、两个已物化 Qwen3 MoE candidates 和 0.5B baseline 都是 `ready_to_host`，trust-region 以及另外两个 MoE candidates 还缺 checkpoint，`1` 个 toy 不可加载。
