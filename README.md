...
最新 MoE 机制结果：toy MoE 上，失败主因不是“平均”这个动作本身，而是 expert 语义 index、router dispatch 和 expert load capacity 一起漂移。当前统一方法 `unified_moe_average` 先用 per-expert source weight search 处理 expert 语义/重要性，再对 router seed 和 capacity loss 系数做 held-out selection sweep；候选 seed 包括 base router、soft-calibrated router、Router-KD router 和 route-KD router。最终选择 `router_kd_seed`、capacity loss `0.0`，soft worst accuracy 为 `0.785`，hard top-2 worst accuracy 为 `0.690`，高于 route-KD 的 `0.685`；max top-k overflow 为 `0.0775`，也略低于 route-KD 的 `0.07875`。进一步的 `unified_output_projection_moe_average` 把 expert seed 换成 route-conditioned output-space projection，再跑同一个 router/capacity selection：soft worst accuracy 升到 `0.795`，max top-k overflow 降到 `0.0700`，但 hard top-2 worst accuracy 小降到 `0.685`，因此它进入 hard top-2/overflow Pareto frontier，而不是单点替代 `unified_moe_average`。`unified_moe_bias_capacity_average` 只训练 router bias，不重学完整 router 几何，把 hard top-2 worst accuracy 保持在 `0.6825`，同时把 max top-k overflow 降到 `0.0475`；`accuracy - overflow` 得到 `0.635`，仍是当前 capacity-aware sparse 推荐。结论是：output-space expert projection 可以改善 soft 能力和容量权衡，但 sparse MoE 的最终选择必须同时看 hard dispatch accuracy、route overlap 和 capacity overflow。这个机制现在已经能写成 checkpoint recipe：`scripts/build_moe_router_bias_plan.py` 从 `expert_load.csv` 生成 `tensor,index,delta`，`scripts/write_same_shape_average_checkpoint.py --tensor-add-csv` 会在保持同构的前提下把 bias delta 写入已有 tensor。

这个结果也解释了为什么不能只机械套一个低-overflow router：`unified_router_kd_seed_average` 把 Router-KD router 直接接到 expert-search 权重上后，hard top-2 code accuracy 只有 `0.5975`，说明 router prior 和 expert 权重必须共同校准。output-space projection probe 也更新为分层结果：它按 base router route probability 给 expert 输出残差加权，mean captured fraction 是 `0.616`；`expert_output_projection_router_calibrated_average` soft worst accuracy 达到 `0.8075`，是当前 soft-router 最优，但 hard top-2 worst accuracy 只有 `0.6475`；把 output-projection expert seed 接入 unified router/capacity objective 后，hard top-2 worst accuracy 恢复到 `0.685`，说明 output-space expert weights 要配合 sparse router objective 才有部署意义。
...
17. **Toy MoE 已经复现 expert-index mismatch 和 router 漂移风险。** [Toy MoE Route-Aware Merge](results/toy_moe_merge/report.md) 中，直接 all-weight average 的 worst accuracy 是 `0.545`，expert-matched average 是 `0.750`，route-aware expert average 是 `0.750`；进一步的 `unified_moe_average` 在 expert 权重搜索后做 router seed/capacity sweep，soft worst accuracy 达到 `0.785`，hard top-2 worst accuracy 达到 `0.690`；`unified_output_projection_moe_average` 用 output-space expert seed 把 soft worst accuracy 提到 `0.795`、overflow 降到 `0.0700`，但 hard top-2 worst accuracy 为 `0.685`；bias-only capacity 修正把 max top-k overflow 从 `0.0775` 降到 `0.0475`，capacity-aware score 达到 `0.635`。
...
当前 coverage audit：`complete = 39`, `partial = 1`, `missing = 0`；唯一 partial 是 generic target-registry vLLM eval 还没有跑完，但 materialized checkpoint、source-vs-merge 对照、probe-guided dense candidate 和 dense guard ablation 的真实 vLLM eval 都已完成。完整汇总见 `results/summary.md` 和 `results/summary.json`。
