...
当前 coverage audit：`complete = 43`, `partial = 1`, `missing = 0`；唯一 partial 是 generic target-registry vLLM eval 还没有跑完，但 materialized checkpoint、source-vs-merge 对照、probe-guided dense candidate 和 dense guard ablation 的真实 vLLM eval 都已完成。完整汇总见 `results/summary.md` 和 `results/summary.json`。
