...
> **最新（机制驱动 unified average，2026-06）：** 见 **[Unified Average Optimizer](results/unified_average_optimizer/report.md)**、**[Qwen3 MoE vLLM Eval Budget Plan](results/qwen3_moe_eval_budget_plan/report.md)**、**[Qwen3 MoE Expert Geometry Probe](results/qwen3_moe_expert_geometry_probe/report.md)**、**[Qwen3 MoE Unified Mechanism Candidate](results/qwen3_moe_unified_mechanism_candidate/report.md)**、**[Qwen3 MoE Mechanism Leverage Map](results/qwen3_moe_mechanism_levers/report.md)** 和 **[Qwen3 MoE Layer/Chunk Coefficient Candidate](results/qwen3_moe_layer_chunk_candidate/report.md)**。这个脚本把 Dense barrier、MoE expert gauge、Qwen3 expert identity、router movement、真实专家内部几何和最终 vLLM 选择 gate 合成一个同构操作策略，而不是静态判断哪个算法名最好。当前证据是：(1) Dense midpoint 的二阶 Fisher 预测严重失效，general/code actual-predicted ratio 为 `42.86/26.66`，所以拒绝默认 `0.5/0.5`；(2) 真实 OLMoE gauge 反事实中，同名 average 让 NLL 从 `4.1678` 升到 `9.6588`，恢复 expert 对齐后回到 `4.1678`；(3) Qwen3-30B-A3B Instruct vs Coder 的 expert identity 通过，`48/48` 层 identity-optimal，但 router direct move gate 为 `0/48` 层通过，所以先 freeze/router-KD gate；(4) 真实 expert geometry probe 读取 `18,432` 个 routed expert projection tensor，mean/p05 cosine 为 `0.386/0.118`、mean/p95 relative delta 为 `1.062/1.275`，发现 `931` 个 high internal-geometry-risk experts 和 `204` 个 high route+geometry-risk experts；(5) 新的 unified mechanism optimizer 选中 `router_evidence_risk_s0.75`，已物化成独立同构 checkpoint，audit 显示 total relative norm `0.240`、router `0/48` changed、attention `0/288` changed、routed max relative delta `0.623`，`>0.65` 为 `0`；(6) 八个 Qwen3 same-shape candidates 仍需同场 vLLM eval，当前 final selector 是 `awaiting_source_eval`、`0/8` eligible；(7) Qwen3 MoE 远端 eval 不再按 `64` 条 smoke 样本硬选，新的预算脚本按 Wilson 95% half-width `0.05` 和 paired sign-test alpha `0.05` 推荐 `384` max examples，10 个 source/candidate 总预算 `15,360` prompts。
