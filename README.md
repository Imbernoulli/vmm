...
> **最新（机制驱动 unified average，2026-06）：** 见 **[Unified Average Optimizer](results/unified_average_optimizer/report.md)**、**[Qwen3 MoE vLLM Eval Budget Plan](results/qwen3_moe_eval_budget_plan/report.md)**、**[Qwen3 MoE Expert Geometry Probe](results/qwen3_moe_expert_geometry_probe/report.md)**、**[Qwen3 MoE Router Calibration NLL Probe](results/qwen3_moe_router_calibration_nll_probe/report.md)**、**[Qwen3 MoE Unified Mechanism Candidate](results/qwen3_moe_unified_mechanism_candidate/report.md)**、**[Qwen3 MoE Mechanism Leverage Map](results/qwen3_moe_mechanism_levers/report.md)** 和 **[Qwen3 MoE Layer/Chunk Coefficient Candidate](results/qwen3_moe_layer_chunk_candidate/report.md)**。这个脚本把 Dense barrier、MoE expert gauge、Qwen3 expert identity、router movement、router margin fragility、router-only NLL calibration、真实专家内部几何和最终 vLLM 选择 gate 合成一个同构操作策略，而不是静态判断哪个算法名最好。当前证据是：(1) Dense midpoint 的二阶 Fisher 预测严重失效，general/code actual-predicted ratio 为 `42.86/26.66`，所以拒绝默认 `0.5/0.5`；(2) 真实 OLMoE gauge 反事实中，同名 average 让 NLL 从 `4.1678` 升到 `9.6588`，恢复 expert 对齐后回到 `4.1678`；(3) Qwen3-30B-A3B Instruct vs Coder 的 expert identity 通过，`48/48` 层 identity-optimal，但 router direct move gate 为 `0/48` 层通过；新的 router margin gate 又显示 `24/48` 层 high-fragility、L17 score `0.752`、min safe-lambda proxy `0.0197`，所以 direct router average 继续 freeze，只允许单独 router-KD/calibration ablation；(4) 真实 expert geometry probe 读取 `18,432` 个 routed expert projection tensor，mean/p05 cosine 为 `0.386/0.118`、mean/p95 relative delta 为 `1.062/1.275`，发现 `931` 个 high internal-geometry-risk experts 和 `204` 个 high route+geometry-risk experts；(5) Qwen3 MoE Instruct/Coder 直线 connectivity 里 best interior worst-NLL 是 `2.5947`，仍差于 best endpoint `2.4757`，Thinking/Coder complementary path 也没有超过 best source，所以不能因为同族/同构就接受 source-to-source midpoint；(6) 真实 router-only NLL probe 只训练 `mlp.gate.weight`，把 linear MoE merge 的 worst-NLL 从 `2.6355` 降到 `2.4140`，code NLL 比两个 source 最好值还低 `0.0139`，说明剩余误差确实有 router dispatch/co-adaptation 成分，但 worst/avg 仍未支配 best source；(7) 新的 unified mechanism optimizer 选中 `router_evidence_risk_s0.75`，已物化成独立同构 checkpoint，audit 显示 total relative norm `0.240`、router `0/48` changed、attention `0/288` changed、routed max relative delta `0.623`，`>0.65` 为 `0`；(8) 九个 Qwen3 same-shape candidates 仍需同场 vLLM eval，其中八个已物化可 host，新增 subspace-scaled ablation 已注册但等待物化，当前 final selector 是 `awaiting_source_eval`、`0/9` eligible；(9) Qwen3 MoE 远端 eval 不再按 `64` 条 smoke 样本硬选，新的预算脚本按 Wilson 95% half-width `0.05` 和 paired sign-test alpha `0.05` 推荐 `384` max examples，当前是 `13` 个 planned source/candidate rows：`10` 个 ready-to-host、`1` 个 subspace checkpoint-missing ablation、`2` 个 router-calibration active caps 等待物化，总 planned budget `19,968` prompts，当前可直接跑的 ready budget `15,360` prompts；`cap0025/cap005` 已由 router margin gate 从默认预算中 plan-prune，新增 `margin_profile` 候选按每层 safe-lambda proxy 给 router delta 设上限。

新增 [Qwen3 MoE Expert Subspace Conflict Probe](results/qwen3_moe_expert_subspace_conflict_probe/report.md) 把 expert identity 之后的内部子空间风险也落成了 gate：它从真实 Qwen3 `18,432` 个 routed expert projection 的 channel/chunk geometry 计算 subspace conflict，发现 `1,323/6,144` 个 expert 是 high subspace-conflict，其中 `242` 个同时 route-important；当前 unified 规则已经覆盖绝大多数高风险点，只剩 `17` 个 expert 需要额外 subspace scale，若物化成 ablation 总共只会减少 Coder weight `0.253078`。这个 ablation recipe 已通过 same-shape writer dry-run：`5,243` 条 expert tensor rules 命中 `15,729` 个 projection tensors，`48` 个 router tensors 继续 freeze；非 dry-run writer command 也已注册进 materialization readiness 和 vLLM eval plan，但 checkpoint 尚未写出。这说明下一步不是盲目整体降权，而是只针对少数“同名 expert 但内部局部子空间冲突仍没被 cap 覆盖”的 expert 做更细 shrink。
...
当前最实在的 Qwen3 MoE 结果是：我已经把 route/load probe、category specialization、router fragility、真实 expert geometry 和真实 delta audit 合成一个同构候选链。现在除了 [searched cap-law candidate](results/qwen3_moe_searched_no_gt065_delta_audit/report.md) 和 geometry-informed [layer/chunk coefficient candidate](results/qwen3_moe_layer_chunk_delta_audit/report.md)，还物化了独立的 [unified mechanism candidate](results/qwen3_moe_unified_mechanism_delta_audit/report.md)：它在 `19` 个候选 cap/prior 里选中 `router_evidence_risk_s0.75`，不是 searched no-gt-0.65 的 alias。真实 safetensors audit 通过：attention `0/288` changed、router `0/48` changed、total relative delta norm `0.240`、routed max relative delta `0.623`、routed `>0.75`/`>0.65`/`>0.6505` 都为 `0`。现阶段不能提前说 unified 一定更好；真正的算法优劣还要看 Qwen3 Instruct/Coder source 和 8 个 same-shape candidates 在同一套下游任务上的表现。

最新 [Qwen3 MoE Delta Frontier Probe](results/qwen3_moe_delta_frontier/report.md) 把八个已物化 same-shape candidates 的真实 safetensors delta 放到同一张表里。结论很明确：route-guarded -> audit-gated 主要消掉 routed expert 的危险大步长（`>1.0` 从 `182` 到 `0`，`>0.75` 从 `839` 到 `164`）；audit-gated -> trust-region 继续把 `>0.75` 从 `164` 压到 `14`；trust-region -> expert-only 不再改善 routed expert tail，只是把 attention delta 从 `0.189` 清零；expert-only -> tail-trimmed 才继续压 routed tail，把 `>0.65` 从 `366` 降到 `80`（剩余都在 `0.6505` rounding slop 内）；searched cap-law 把复杂 penalty 换成统一 `0.65` cap，total relative norm `0.248`；geometry-informed layer/chunk 继续把 total relative norm 降到 `0.243`，strict `>0.65` 降到 `89`；新的 unified mechanism candidate 再把 total relative norm 降到 `0.240`，并把 routed `>0.65` 从 `89` 降到 `0`。下一版 unified MoE 规则里，trust-region/tail-trim 是安全机制，attention、risk penalty、layer sensitivity 和 geometry-aware shrink 是否保留都必须交给同任务 vLLM eval 决定。

最新 [Qwen3 MoE Mechanism-Gated vLLM Eval Gate](results/qwen3_moe_mechanism_eval_gate/report.md) 已把这个问题转成可执行评测：两个 source endpoint 加八个已物化 same-shape Qwen3 MoE candidates 是 `ready_to_host`，第九个 `qwen3_moe_subspace_scaled_candidate` 已进入 gate 但状态是 `checkpoint_missing_until_materialized`。这个 gate 不再问“哪个算法名最好”，而是逐项检验 tail delta cap、route/load trust-region、shared attention ablation、0.65 tail trim、risk penalty simplification、layer/chunk sensitivity、unified mechanism optimizer、subspace conflict ablation 和 endpoint fallback；如果所有 average 候选被 source 支配，selection rule 会返回同构 endpoint/no-average。当前本机 `nvidia-smi` 不可用，所以 selection 状态是 `awaiting_remote_vllm_eval`。

最新 [Qwen3 MoE vLLM Eval Budget Plan](results/qwen3_moe_eval_budget_plan/report.md) 把“正当评测要跑多少”也落成脚本：原 gate 的 `64` examples 只保留为 audit floor，预算版 `run_eval_budget.sh` 会把 ready-to-host 的十个 source/candidate 的 `--max-examples` 提到 `384`；同时预算表把 subspace-scaled ablation 和 router calibration 的 active `cap001`/`margin_profile` 纳入 planned queue，但因为 checkpoint 尚未物化，runner 现在会跳过它们。这个数来自两个约束：binary task score 的 95% Wilson half-width 要压到 `0.05` 以内需要 `381` 条，四舍五入到 `384`；paired prediction gate 若要在 alpha `0.05` 下看出约 `5pp` 的净 source advantage，按 `25%` discordance 估计需要约 `248` 条 shared examples。新的 planned prompt budget 是 `19,968`，其中当前可直接 host 的 ready budget 是 `15,360`；active/ready/pending/plan-pruned router caps 是 `2/0/2/2`。因此下一轮远端 vLLM 不是静态 smoke，也不会默认浪费在 margin gate 已拒绝的 `cap0025/cap005` 上，而是能支撑 source dominance、task regression、score-confidence 和 paired-prediction regression 的机制评测；HumanEval 受 `164` 条数据上限限制，selector 会按实际样本数计算区间。
...
32. **Checkpoint materialization readiness 已推进到八个 Qwen3 checkpoint candidates 可评测，并把第九个 subspace ablation 放入待物化队列。** [vLLM Checkpoint Eval Plan](results/vllm_checkpoint_eval_plan/report.md) 显示 Qwen3 Instruct source、Qwen3 Coder source、八个已物化 Qwen3 MoE candidates 和 0.5B baseline 都是 `ready_to_host`；`qwen3_moe_subspace_scaled_candidate` 指向 `results/checkpoints/qwen3_moe_subspace_scaled_candidate`，但当前是 `checkpoint_missing_until_materialized`。其中 `qwen3_moe_unified_mechanism_candidate` 指向独立 checkpoint `results/checkpoints/qwen3_moe_unified_mechanism_candidate`，不再复用 searched no-gt-0.65。最新 [Unified Result Selector](results/qwen3_moe_unified_result_selection/report.md) 仍是 `awaiting_source_eval`，所以现在不会提前宣布 average 胜出。
