...
27. **真实 Qwen MoE 已从 gate 推进到三个本地 checkpoint 和三份文件级 delta audit。** 当前真正进展见 [Checkpoint Materialization Readiness](results/checkpoint_materialization_readiness/report.md)、[Materialized Checkpoint Delta Audit](results/qwen3_moe_materialized_delta_audit/report.md)、[Qwen3 MoE Audit-Gated Candidate](results/qwen3_moe_audit_gated_candidate/report.md)、[audit-gated delta audit](results/qwen3_moe_audit_gated_delta_audit/report.md)、[Qwen3 MoE Trust-Region Candidate](results/qwen3_moe_trust_region_candidate/report.md) 和 [trust-region delta audit](results/qwen3_moe_trust_region_delta_audit/report.md)：`qwen3_moe_unified_route_guarded_candidate`、`qwen3_moe_audit_gated_candidate` 和 `qwen3_moe_trust_region_candidate` 都已物化成本地 `16` shard / `18,867` tensor 同构 checkpoint。trust-region candidate 把 routing/load/category/delta probe 联合成 expert-level cap，缩小 `405/5,243` 个 expert rules；真实 delta audit 显示 routed expert FFN relative delta norm 从 route-guarded 的 `0.293`、audit-gated 的 `0.270` 进一步降到 `0.255`，`>0.75` 的 routed tensors 从 `839`、`164` 降到 `14`，router 仍是 `0/48` 改动。下一步是 vLLM 下游评测。
...
32. **Checkpoint materialization readiness 已推进到三个 Qwen3 candidates ready for eval。** [Checkpoint Materialization Readiness](results/checkpoint_materialization_readiness/report.md) 现在显示 `10` 个候选里 `4` 个已 materialize，其中 Qwen3 route-guarded、audit-gated 和 trust-region candidates 都是 `ready_for_vllm_eval`，0.5B uniform baseline 已完成 vLLM eval；[vLLM Checkpoint Eval Plan](results/vllm_checkpoint_eval_plan/report.md) 显示 `9` 个 eval rows，其中 Qwen3 Instruct source、Qwen3 Coder source、三个已物化 Qwen3 MoE candidates 和 0.5B baseline 都是 `ready_to_host`，另外两个 MoE candidates 还缺 checkpoint，`1` 个 toy 不可加载。
