...
- Qwen source discovery: `source_discovery_plan_ready`，top scenario `dense_7b_general_code_math_reasoning`，top queue `measured_coder_thinking_endpoint_expansion`，additional frontier avg gain needed `0.0611`。
- Qwen source discovery eval: `source_discovery_vllm_eval_plan_ready`，jobs `4`，top job `measured_coder_thinking_source_frontier`，task names `['gsm8k', 'humaneval_compile', 'mmlu', 'safety']`，compatibility `passed_humaneval_compile_task_name`。
- Qwen3 router calibration frontier: `router_calibration_frontier_ready`，default `2/4`，recommended `['cap001', 'margin_profile']`，blocker `baseline_eval,source_eval,candidate_eval,audit,group_validation,capacity_metrics`，nll `0.2214`，generation `0.0333`。
...
| `moe` | `qwen_source_discovery_plan` | `search_stronger_source_sets_before_more_average_tuning` | 0.0611 | 0.0000 | status = source_discovery_plan_ready; top measured set = coder+thinking; additional avg frontier gain needed = 0.0611; top scenario = dense_7b_general_code_math_reasoning; top queue = measured_coder_thinking_endpoint_expansion |
| `moe` | `qwen_source_discovery_vllm_eval_plan` | `source_frontier_eval_jobs_are_materialized_before_average_acceptance` | 4.0000 | 1.0000 | status = source_discovery_vllm_eval_plan_ready; jobs = 4; task names = ['gsm8k', 'humaneval_compile', 'mmlu', 'safety']; task compatibility = passed_humaneval_compile_task_name; top job = measured_coder_thinking_source_frontier |
| `moe` | `qwen3_router_calibration_frontier` | `router_calibration_is_margin_frontier_not_default_router_average` | 2.0000 | 1.0000 | status = router_calibration_frontier_ready; default candidates = 2/4; recommended = ['cap001', 'margin_profile']; safe lambda = 0.0197; nll signal = 0.2214; generation signal = 0.0333; blocker = baseline_eval,source_eval,candidate_eval,audit,group_validation,capacity_metrics |
...
| `moe_router_calibration_frontier_gate` | `restrict router calibration to margin-safe frontier probes` | run default router-cal frontier candidates [cap001,margin_profile] only as ablations; stress caps stay non-default; blocker=baseline_eval,source_eval,candidate_eval,audit,group_validation,capacity_metrics | It turns router calibration into a bounded same-shape repair test instead of reintroducing unsafe direct router averaging. |
