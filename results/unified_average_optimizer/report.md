...
- Qwen3 unified mechanism: `subspace_cap_s1.00`；retention `0.9763`，subspace-weighted rel-delta `0.2148`，high-subspace mean scale `0.9614`，materialized rules `stale_or_different_rules`，audit relative norm `0.2396`，routed >0.65 `0`。
...
| `moe` | `qwen3_unified_materialized_audit` | `materialized_same_shape_tail_reduction` | 0.2396 | 0.2435 | rule status = stale_or_different_rules; audit status = passed; total relative norm = 0.2396; router changed = 0/48; layer/chunk->unified norm reduction = 0.0038; routed >0.65 reduction = 89 |
