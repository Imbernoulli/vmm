...
- Toy MoE sanity check：用 [results/toy_moe_merge/report.md](results/toy_moe_merge/report.md) 和 [results/toy_moe_routing_readiness/report.md](results/toy_moe_routing_readiness/report.md) 先在可控同构 MoE 上验证 expert-index mismatch、expert-output matching、route-frequency expert average 以及多方法 readiness gate；真实 Qwen3 MoE 实验必须至少复现同类 probe 表。
