...
- MoE routing probe：用 `scripts/probe_moe_routing.py` 捕获真实 Qwen3 MoE router hook，写出 `router_summary.csv`、`expert_load.csv`、可选 `route_overlap.csv`、`summary.json` 和 `report.md`；tiny smoke 见 [results/moe_routing_probe_smoke/report.md](results/moe_routing_probe_smoke/report.md)，下一步把它跑在 Qwen3-30B-A3B 与 Qwen3-Coder-30B-A3B 上。
