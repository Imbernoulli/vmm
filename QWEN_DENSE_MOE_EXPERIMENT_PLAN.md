...
这份方案把后续实验范围从“官方 Base vs Instruct”收紧并升级为：围绕 Qwen 官方底座、官方专长分支、以及下游用户/实验室基于 Qwen 继续 SFT/RL/蒸馏得到的模型，系统比较 Dense 模型和 MoE 模型上的 model averaging、线性插值、task arithmetic、TIES/DARE 等合并方法。
...
3. **Adapter/MoLE 风格合并**：如果下游模型主要是 LoRA/adapters，优先在 adapter 层做 mixture-of-LoRA-experts 或 route-aware adapter merge，而不是改完整 MoE 权重。
...
1. **模型资格检查**：确认 architecture、hidden size、layer count、vocab/tokenizer、chat template 是否可对齐。不能对齐的模型只做 LoRA/adapter merge 或输出层适配，不做直接 full-weight merge。
...
| expert FFN | expert matching 后合并，或保留多 expert | MoE 专长主要在 FFN experts。 |
...
5. **Expert union / sparse branch retention**：冲突大的 expert 不平均，而是保留成额外 experts，通过 router 选择。
6. **MoLE / adapter MoE**：如果下游是 LoRA，优先把 LoRA 当专家，通过输入自适应路由，而不是压成一个 delta。
7. **MergeMoE-style expert output merge**：把 expert merging 看成输出空间压缩/拟合，适合做 MoE 压缩和专家合并。
