...
toy MoE 的作用是把这个逻辑先跑通：在保持同构的前提下，code source 被做了函数等价的 expert permutation。直接 all-weight average 的 worst accuracy 只有 `0.545`，expert-output matching 后达到 `0.750`，route-frequency expert average 也是 `0.750`。进一步的 output-space projection + router calibration 在 soft-router 下达到 `0.8075`，但 hard top-2 worst accuracy 只有 `0.6475`；`unified_moe_average` 在 expert 权重搜索后做 router/capacity selection，hard top-2 worst accuracy 达到 `0.690`；`unified_output_projection_moe_average` 把 expert seed 换成 output-space projection 后，soft worst accuracy 是 `0.795`、hard top-2 worst accuracy 是 `0.685`、max top-k overflow 从 `0.0775` 降到 `0.0700`，进入 hard top-2/overflow Pareto frontier。这个结果证明后续 Qwen3 MoE 实验必须同时记录 expert index、router overlap、expert load、route mass 和 hard sparse dispatch 指标。
...
[Toy MoE Method Selection](results/toy_moe_method_selection/report.md) 则把这一步再往前推：它把 method metrics 和 routing readiness 合成 `baseline_only`、`reject_routing_breakdown`、`reject_underperforms_base`、`candidate_with_router_guard` 等决策。当前 toy MoE 中，`all_weight_average` 被拒绝为 routing breakdown；soft-router 推荐 `expert_output_projection_router_calibrated_average`；hard top-2 sparse 推荐 `unified_moe_average`；capacity-aware sparse 推荐 `unified_moe_bias_capacity_average`。`unified_output_projection_moe_average` 虽然不是单点推荐，但进入了 hard top-2 accuracy / overflow Pareto frontier，说明 output-space expert weights 需要和 sparse router/capacity gate 一起评估。这就是后续 Qwen3 MoE 应该复制的决策闭环。
