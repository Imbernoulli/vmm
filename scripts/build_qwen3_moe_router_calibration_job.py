...
def build_run_script(
    ...
    default_labels = [row["cap_label"] for row in candidate_rows if row.get("default_run_enabled")]
    for label in default_labels:
        lines.append(f"  run_{label}")
    for row in candidate_rows:
        if not row.get("default_run_enabled"):
            lines.append(f"  # skipped {row['cap_label']}: router margin planned cap failed")
